...
monet  = applications.mobilenet.MobileNet(weights='imagenet', include_top= False, dropout=.3, input_shape=(224,224,3))
#
...
#
# data_x.reshape(-1,224,224,3)
# training
# with open('split_monet_train_data.pickle', 'wb') as f:
#     pickle.dump(data_x,f, protocol=4)
# with open('split_monet_train_label.pickle', 'wb') as f:
#     pickle.dump(trY,f, protocol=4)
# we load train features
# with open('split_monet_train_data.pickle', 'rb') as f:
#     train_f_data = pickle.load(f)
# with open('split_monet_train_label.pickle', 'rb') as f:
#     train_f_label = pickle.load(f)
#
# del data_x
#
...
def creat_model():
    model = Sequential()
    model.add(TimeDistributed(Conv2D(256, 3, 3, activation='relu'), input_shape=(None,224,224,3)))
    model.add(TimeDistributed(MaxPooling2D()))
    model.add(TimeDistributed(Flatten()))
    model.add(LSTM(50)) #, return_sequences=False, kernel_initializer="uniform",
               #forget_bias_init='one', inner_activation='sigmoid',
